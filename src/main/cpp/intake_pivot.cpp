...
//Switching Pos
	...
	if (joy1->GetRawButton(4)==1 and toggle1 ==0) {
	current_pivot_pos = current_pivot_pos +1;
	toggle1 = 1;
	}
	if (joy1->GetRawButton(4)==0 and toggle1 ==1){
	toggle1 = 0;
	}
	if (joy1->GetRawButton(3)==1 and toggle2==0) {
	current_pivot_pos = current_pivot_pos -1;
	toggle2=1;
	}
	if (joy1->GetRawButton(3)==0 and toggle2 ==1){
	toggle2 = 0;
	}
...
if (mode ==1){
	claw_pivot_talon_enc->Set(ControlMode::PercentOutput, joy1->GetRawAxis(5) *joy1->GetRawAxis(5)* joy1->GetRawAxis(5));
